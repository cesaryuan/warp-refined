...
当前仓库默认构建不再开启 orchestration 相关能力。后续如果看到 `OrchestrationV2` / `RunAgentsTool` / orchestration pill bar / orchestration launch modal 相关逻辑没有生效，先确认是不是被显式加回了 `app/Cargo.toml` 的默认 feature，而不要默认以为是运行时 bug。
